...
function bundlePage(constants: { [key: string]: unknown }) {
	const bundle = fs.readFileSync(path.join(bundleDir, 'client.bundle.js'), 'utf-8');
...
function test_profile_time(base: string, elf: string) {
	...
	const html = bundlePage({ MODEL: model });

...
export type FindRef = (action: string, text?: string) => void;
const findFn: ForwardFn<{
	callback: (args: FindCallback) => void;
	curFind: number;
	findResultLength: number;
}, FindRef> = ({ curFind, findResultLength, callback }, ref) => {
	...
	const onFindPaste = useCallback((evt: JSX.TargetedEvent<HTMLInputElement>) => {
		callback({ text: evt.currentTarget.value });
	}, [callback]);
	...
	const onFindKeyDown = useCallback((evt: KeyboardEvent) => {
		if(evt.key === 'Enter')
			callback({ action: evt.shiftKey ? 'prev' : 'next' });
		if(evt.key === 'Escape') {
			// close search bar
			divRef.current.getElementsByTagName('input')[0].blur();
			divRef.current.classList.remove(styles.find_visible);
	...
	return (<div ref={divRef} class={[styles.find, styles.find_hidden].join(' ')}>
		...
		<button class={styles.button} onMouseDown={() => callback({ action: 'prev' })} disabled={findResultLength === 0} type="button" title="Previous match (Shift+Enter)" dangerouslySetInnerHTML={{ __html: ChevronUp }} />
		<button class={styles.button} onMouseDown={() => callback({ action: 'next' })} disabled={findResultLength === 0} type="button" title="Next match (Enter)" dangerouslySetInnerHTML={{ __html: ChevronDown }} />

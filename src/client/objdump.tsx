...
//					break;
			...
			if(insnMatch) {
				...
				if(jump) {
					const jumpInfo = jumps.find((j) => j.end === jump.target && j.type === jump.type);
					if(jumpInfo)
						jumpInfo.start.push(pc);
					else
						jumps.push({ start: [pc], end: jump.target, level: -1, type: jump.type });
				}
		...
		const sortedJumps = jumps.sort((a: JumpInfo, b: JumpInfo) => {
		...
		for(const jump of sortedJumps) {
			...
			for(const startPc of jump.start) {
				const start = pcMap.get(startPc) - min;
